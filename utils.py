...
#bbox im Format x1, y1, x2, y2 (linke obere ecke, rechte untere ecke)
def process_to_xml(datapoints, data_path):
    for i in range(0, len(datapoints)):
        datapoint = datapoints[i]
        doc = minidom.Document()
        annotations = doc.createElement('annotation')
        ...
        filename.appendChild(doc.createTextNode(datapoint.image.file_name))
        annotations.appendChild(filename)

        path = doc.createElement('path')
        path.appendChild(doc.createTextNode(datapoint.image.file_name))
        annotations.appendChild(path)

        source = doc.createElement('soruce')
        ...
        width.appendChild(doc.createTextNode(str(datapoint.image.width)))
        height = doc.createElement('height')
        height.appendChild(doc.createTextNode(str(datapoint.image.height)))
        ...
        for j in range(0, len(datapoint.anns)):
            ...
            name.appendChild(doc.createTextNode('rubbish'))
        ...
        image_name_xml = datapoint.image.file_name.replace("jpg", "xml")
        path = os.path.join(data_path, image_name_xml)
...
def showbbox(img_path, datapoint):
    path = os.path.join(img_path, datapoint.image.file_name)
    image_bbox = read_image(path)
    imgbb = Image.open(path)
